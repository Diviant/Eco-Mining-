...
const EconomySection: React.FC = () => {
  return (
    <section id="economy" className="py-32 bg-slate-50 dark:bg-slate-950/50 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <div className="text-emerald-600 dark:text-emerald-400 font-black text-xs uppercase tracking-[0.3em] mb-4">Financial Model</div>
          <h2 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white mb-4 tracking-tighter italic">Экономика <span className="text-emerald-600 dark:text-emerald-500">Complex-X</span></h2>
          <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto">Двойная диверсификация рисков: высокодоходный цифровой актив и стабильный товарный рынок FMCG.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          {[
            ...
            { label: "Годовой ROI", value: "84%", desc: "С учетом роста сложности сети", icon: <PieChart className="text-emerald-600 dark:text-emerald-400" /> }
          ].map((item, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-black flex flex-col items-center text-center group hover:-translate-y-2 transition-all">
              <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                {item.icon}
              </div>
              <h4 className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest mb-2">{item.label}</h4>
              <div className="text-4xl font-black mb-3 text-slate-900 dark:text-white">{item.value}</div>
              <p className="text-slate-500 dark:text-slate-400 text-xs font-bold leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data} margin={{ top: 20, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <Tooltip 
                    ...
                    contentStyle={{ borderRadius: '24px', border: 'none', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', backgroundColor: '#0f172a', color: '#fff' }}
                  />
          ...
          <div className="bg-slate-900 dark:bg-black text-white p-10 rounded-[3rem] shadow-2xl flex flex-col justify-center border border-white/5">
