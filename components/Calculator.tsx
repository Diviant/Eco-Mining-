...
import { Sliders, TrendingUp, Zap, Sprout, CheckCircle2, ArrowRight } from 'lucide-react';
    ...
    <section id="calculator" className="py-32 bg-slate-50 dark:bg-slate-900/50 relative overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xl max-w-2xl mx-auto font-medium">Рассчитайте параметры вашего будущего комплекса в режиме реального времени.</p>
        </div>

        <div className="grid lg:grid-cols-2 gap-8 items-stretch">
          <div className="bg-white dark:bg-slate-900 p-8 lg:p-12 rounded-[3.5rem] shadow-xl border border-slate-100 dark:border-slate-800 space-y-12 transition-colors">
            <div>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-800 px-4 py-1 rounded-full text-2xl font-black text-slate-900 dark:text-white transition-colors">{power}</div>
              </div>
              <input 
              ...
              />
              <div className="flex justify-between text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest">
                <span>100 кВт</span>
                <span>2 МВт</span>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-800 px-4 py-1 rounded-full text-2xl font-black text-slate-900 dark:text-white transition-colors">{gasPrice} <span className="text-sm opacity-30 dark:opacity-20 italic">₽</span></div>
              </div>
              <input 
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {(['onion', 'basil', 'microgreens'] as const).map((type) => (
                  ...
                  >
                    <div className={`p-2 rounded-xl ${cropType === type ? 'bg-white/20' : 'bg-slate-50 dark:bg-slate-800'}`}>
                      <Sprout size={16} />
                    </div>
          ...
          <div className="bg-slate-900 dark:bg-black rounded-[3.5rem] p-10 lg:p-14 text-white flex flex-col justify-between shadow-2xl relative overflow-hidden group border border-white/5 transition-colors">
            ...
            <div className={`space-y-12 transition-all duration-700 ${isCalculated ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
              <div>
                ...
                <div className="text-6xl md:text-7xl font-black tracking-tighter">
                  {results.profit.toLocaleString()} <span className="text-2xl text-emerald-500 dark:text-emerald-400">₽/мес</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10 transition-colors">
              ...
              <div className="bg-white/5 p-8 rounded-[2rem] border border-white/10 flex items-center justify-between transition-colors">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Срок окупаемости CAPEX</div>
                  <div className="text-3xl font-black text-emerald-500 dark:text-emerald-400 italic">{results.payback} <span className="text-lg">месяцев</span></div>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-emerald-500 dark:bg-emerald-600 flex items-center justify-center text-slate-900 dark:text-white transition-colors">
                  <ArrowRight />
                </div>
              </div>
            </div>
            ...
            <div className="mt-12">
              <button className="w-full bg-emerald-600 dark:bg-emerald-700 hover:bg-emerald-500 dark:hover:bg-emerald-600 py-7 rounded-3xl font-black text-xl transition-all shadow-xl shadow-emerald-600/20 active:scale-[0.98] flex items-center justify-center space-x-3">
                <CheckCircle2 />
